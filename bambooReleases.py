...
class BambooReleases(object):
	...
	def queryDB(self, cur):
		...
		query = """ SELECT dp.name AS deploy_name FROM deployment_project dp, deployment_environment de,
		            ...
		            de.environment_id = dr.environment_id AND upper(de.name)='PROD' AND
...
if __name__ == '__main__':
	try:
		qproject = sys.argv[1]
	expect IndexError:
	    print("IndexError: You need to pass the project name as first argument!")
	    sys.exit(1)
	bRel = BambooReleases("bamboo", "pg-db01.corp.sea1.cmates.com","test","test123")
	conn = bRel.dbConnect()
	cur  = conn.cursor()
